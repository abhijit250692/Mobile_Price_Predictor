...
def preprocess_input(weight, resolution, ppi, cpu_core, cpu_freq, rear_cam, battery, thickness, internal_mem, ram, front_cam):
	internal_mem_log = np.log1p(internal_mem)
	ram_sqrt = np.sqrt(ram)
	front_cam_log = np.log1p(front_cam)
	return np.array([[weight, resolution, ppi, cpu_core, cpu_freq, rear_cam, battery, thickness, internal_mem_log, ram_sqrt, front_cam_log]])
...
def main():
	...
	with st.form("input_form"):
		...
		with col2:
			...
			battery = st.number_input("Battery (mAh)", min_value=800.0, max_value=10000.0, value=3000.0, step=50.0)
		submitted = st.form_submit_button("Predict Price")
	if submitted:
		input_data = preprocess_input(weight, resolution, ppi, cpu_core, cpu_freq, rear_cam, battery, thickness, internal_mem, ram, front_cam)
